...
class UploadModal extends Component {
    ...
    constructor(props) {
        ...
        this.state = { error: '', file: [], title: '', desc: '', uploading: false };
    }
    ...
    clear() {
        this.setState({error: '', file: [], title: '', desc: '', uploading: false});
    }
    ...
    onDrop(file) {
        ...
        } else if(file[0].size > 5e8) {
            this.setState({error: 'File size is too big. 500 MB or less'})
        } else {
            this.setState({file, error: ''});
        }
    }
    ...
    renderDropContent() {
        ...
        if(this.state.file.length > 0) {
            ...
            return(
                <div className="py-5">
                <div className="h5">Click or drop a file here.<br/>
                <small className="text-muted">(Format: .mp3, .flac)</small>
                </div>
                </div>
            );
        }
    ...
    renderError() {
        if(this.state.error !== '')  {
            ...
            return <div className="alert alert-success"><strong>Fantastic!</strong> Your track is uploading. It will show up on the My Tracks table in a moment.</div>;
        }
    }
